...
<x-ui.table :headers="['Invoice Info', 'Siswa', 'Paket & Harga', 'Tanggal', 'Status', 'Aksi']" :paginator="$transactions">
    @forelse ($transactions as $transaction)
    <x-ui.tr>
        {{-- Kolom 1: Invoice --}}
        <x-ui.td class="whitespace-nowrap">
            <div class="flex items-center gap-3">
                <div class="p-2 bg-indigo-50 text-indigo-600 rounded-lg group-hover:bg-indigo-100 transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                </div>
                <div>
                    <span class="block font-bold text-gray-800">{{ $transaction->invoice_code }}</span>
        ...
        {{-- Kolom 4: Tanggal --}}
        <x-ui.td class="whitespace-nowrap text-gray-600">
        ...
        {{-- Kolom 6: Aksi --}}
        <x-ui.td class="text-right whitespace-nowrap text-sm font-medium">
            <a href="{{ route('admin.transactions.show', $transaction) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors duration-200">
                Detail
            </a>
        </x-ui.td>
    </x-ui.tr>
    @empty
    <x-ui.tr>
        <x-ui.td colspan="6" class="text-center py-12">
            <div class="flex flex-col items-center justify-center">
                ...
                <p class="text-gray-500 mt-1 max-w-sm">Data transaksi pembayaran siswa akan muncul di sini.</p>
            </div>
        </x-ui.td>
    </x-ui.tr>
    @endforelse
