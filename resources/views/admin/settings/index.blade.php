                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div x-show="activeTab === 'general'" class="space-y-4">
                            ...
                            <div>
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['site_', '_'], ['',' '], $setting->key))" />

                                @if($setting->type === 'image')
                                <div class="mt-2"
                                    x-data="{ preview: '{{ $setting->value ? (str_contains($setting->value, 'http') ? $setting->value : asset('storage/' . $setting->value)) : '' }}' }">
                                    ...
                                    <div
                                        class="mt-2 relative inline-block p-2 bg-gray-100 rounded border border-gray-200">
                                        <img :src="preview ? preview : '{{ asset('img/image.png') }}'"
                                            class="h-20 w-auto object-contain">
                                    </div>

                                    @if($setting->value)
                                    <div class="mt-2 flex items-center">
                                        <input type="checkbox" id="delete_{{ $setting->key }}" name="delete_buttons[]"
                                            value="{{ $setting->key }}"
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <label for="delete_{{ $setting->key }}"
                                            class="ml-2 text-sm text-red-600 font-medium">Hapus Logo & Gunakan
                                            Default</label>
                                    </div>
                                    @endif
                                </div>
                                @else
                                <x-inputs.text id="{{ $setting->key }}" name="{{ $setting->key }}" type="text"
                                    class="mt-1 block w-full" :value="old($setting->key, $setting->value)" />
                                @endif
                        ...
                        <div x-show="activeTab === 'hero'" class="space-y-4">
                            <h3 class="text-lg font-bold text-gray-800 mb-4">Pengaturan Hero Section (Halaman Depan)
                            </h3>

                            @foreach($settings['hero'] ?? [] as $setting)
                            <div>
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['hero_', '_'], ['',' '], $setting->key))" />
                                ...
                                <div class="mt-2"
                                    ...
                                    <div x-show="preview" class="mt-2 relative inline-block">
                                        <img :src="preview"
                                            class="h-40 w-auto rounded-lg shadow-sm object-cover border border-gray-200">
                                    </div>
                                    ...
                                    <div class="mt-2 flex items-center">
                                        <input type="checkbox" id="delete_{{ $setting->key }}" name="delete_buttons[]"
                                            ...
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <label for="delete_{{ $setting->key }}"
                                            class="ml-2 text-sm text-red-600 font-medium">Hapus & Reset ke
                                            Default</label>
                                    </div>
                                    @endif
                                </div>
                        ...
                        <div x-show="activeTab === 'features'" class="space-y-4" style="display: none;">
                            ...
                            <p class="text-sm text-gray-500 mb-4">Mengatur teks pada 3 kartu keunggulan di halaman depan
                                (Tentang Kami).</p>

                            @foreach($settings['features'] ?? [] as $setting)
                            <div>
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['feature_', '_'], ['Fitur ',' '], $setting->key))" />
                        ...
                        <div x-show="activeTab === 'faq'" class="space-y-4" style="display: none;">
                            ...
                            <p class="text-sm text-gray-500 mb-4">Mengatur daftar pertanyaan dan jawaban yang sering
                                ditanyakan.</p>

                            @foreach($settings['faq'] ?? [] as $setting)
                            <div class="border-b border-gray-100 pb-6 mb-6 last:border-0 last:mb-0 last:pb-0">
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['faq_', '_'], ['',' '], $setting->key == 'faq_data' ? 'Daftar Pertanyaan (FAQ)' : $setting->key))" />

                                @if($setting->type === 'json_list')
                                <div x-data="{ items: {{ $setting->value ? $setting->value : '[]' }} }">
                                    {{-- Hidden Input for Form Submission --}}
                                    <textarea name="{{ $setting->key }}" class="hidden"
                                        :value="JSON.stringify(items)"></textarea>

                                    {{-- List Items --}}
                                    <div class="space-y-4 mt-2">
                                        <template x-for="(item, index) in items" :key="index">
                                            <div
                                                class="flex gap-4 items-start bg-gray-50 p-4 rounded-lg border border-gray-200">
                                                <div class="flex-1 space-y-3">
                                                    <div>
                                                        <label
                                                            class="text-xs text-gray-500 font-bold uppercase">Pertanyaan</label>
                                                        <input type="text" x-model="item.question"
                                                            ...
                                                            placeholder="Contoh: Apakah ada garansi?">
                                                    </div>
                                                    <div>
                                                        <label
                                                            class="text-xs text-gray-500 font-bold uppercase">Jawaban</label>
                                    ...
                                    <button type="button" @click="items.push({ question: '', answer: '' })"
                                        class="mt-4 flex items-center gap-2 text-sm font-bold text-indigo-600 hover:text-indigo-800">
                                ...
                                @else
                                <x-inputs.text id="{{ $setting->key }}" name="{{ $setting->key }}" type="text"
                                    class="mt-1 block w-full bg-gray-50" :value="old($setting->key, $setting->value)"
                        ...
                        <div x-show="activeTab === 'about'" class="space-y-4" style="display: none;">
                            ...
                            <div>
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['about_', '_'], ['',' '], $setting->key))" />
                        ...
                        <div x-show="activeTab === 'contact'" class="space-y-4" style="display: none;">
                            ...
                            <div>
                                <x-inputs.label :for="$setting->key"
                                    :value="ucwords(str_replace(['contact_', '_'], ['',' '], $setting->key))" />
                                <x-inputs.text id="{{ $setting->key }}" name="{{ $setting->key }}" type="text"
                                    class="mt-1 block w-full" :value="old($setting->key, $setting->value)" />
                            </div>
                            @endforeach
