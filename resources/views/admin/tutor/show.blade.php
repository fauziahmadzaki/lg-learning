@php
    $breadcrumbs = [
        'Manajemen Tutor' => route('admin.tutors.index'),
        $tutor->user->name => null,
    ];
@endphp

<x-app-layout :breadcrumbs="$breadcrumbs">
    <x-slot name="pageTitle">Detail Tutor: {{ $tutor->user->name }}</x-slot>
            ...
            <div class="px-6 pb-6">
                <div class="relative flex justify-between items-end -mt-12 mb-4">
                    ...
                    <div class="mb-2 hidden md:block">
                        <a href="{{ route('admin.tutors.edit', $tutor) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            @if($tutor->phone)
                                <div class="mt-6">
                                    @php
                                        $waNumber = $tutor->phone;
                                        // Simple formatter: 08x -> 628x
                                        if (substr($waNumber, 0, 1) == '0') {
                                            $waNumber = '62' . substr($waNumber, 1);
                                        }
                                        $waLink = "https://wa.me/{$waNumber}?text=".urlencode("Halo {$tutor->user->name}, saya dari admin...");
                                    @endphp
                                    <a href="{{ $waLink }}" target="_blank" class="flex items-center justify-center w-full px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition shadow-sm gap-2 font-medium">
                    ...
                    <div>
                        ...
                        @if($tutor->packages->count() > 0)
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @foreach($tutor->packages as $package)
                                    <a href="{{ route('admin.packages.show', $package) }}" class="block group">
                        ...
                        @else
                            <div class="bg-gray-50 border border-dashed border-gray-200 rounded-lg p-8 text-center">
                                ...
                                <p class="text-sm text-gray-500">Belum ada paket yang ditambahkan untuk tutor ini.</p>
                                <a href="{{ route('admin.tutors.edit', $tutor) }}" class="text-xs text-indigo-600 hover:underline mt-1 block">Tambah Paket &rarr;</a>
                            </div>
                        @endif
                    </div>
                </div>
