        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100 p-8">
                ...
                <form action="{{ route('admin.contents.update', $content) }}" method="POST" enctype="multipart/form-data" 
                    x-data="{ 
                        isCarousel: {{ $content->is_carousel ? 'true' : 'false' }},
                            ...
                            <div class="border-2 border-dashed border-indigo-200 rounded-lg p-4 flex flex-col items-center justify-center min-h-[200px] bg-white"
                                ...
                                <template x-if="imagePreview">
                                    <div class="relative w-full h-full group">
                                        ...
                                        {{-- Only show remove button if it's a new upload --}}
                                         <button type="button" @click="imagePreview = '{{ $content->image_url }}'; document.getElementById('image').value = ''" 
                                            class="absolute top-2 right-2 bg-yellow-500 text-white rounded-full p-1 shadow-md hover:bg-yellow-600 opacity-0 group-hover:opacity-100 transition-opacity" title="Reset ke gambar awal">
