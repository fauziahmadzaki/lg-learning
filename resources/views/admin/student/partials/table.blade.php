    ...
    <x-ui.tr>
        ...
        <x-ui.td>
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <a href="{{ route('admin.students.show', $student) }}" class="text-sm font-medium text-gray-900 hover:text-indigo-600 hover:underline">
                        {{ $student->name }}
                    </a>
        ...
        <x-ui.td class="text-right font-medium">
            <div class="flex items-center justify-end gap-2">
                <a href="{{ route('admin.students.show', $student) }}" class="text-blue-600 hover:text-blue-900 bg-blue-50 p-1.5 rounded-lg hover:bg-blue-100 transition" title="Lihat Detail">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                </a>
                <a href="{{ route('admin.students.edit', $student) }}" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-1.5 rounded-lg hover:bg-indigo-100 transition" title="Edit Data">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                    </svg>
                </a>
                <button @click.prevent="$dispatch('trigger-delete-modal', { url: '{{ route('admin.students.destroy', $student) }}', name: '{{ addslashes($student->name) }}' })" class="text-red-600 hover:text-red-900 bg-red-50 p-1.5 rounded-lg hover:bg-red-100 transition" title="Hapus Data">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
